...
function _normalizeTimeMs(time) {
  ...
  // 与 player.getDurationSec / tracklist.getDurationMs 同步：< 5000ms 视为试听片段/错误值，
  // 丢弃返回 0，避免"1/1"显示
  return ms < 5000 ? 0 : ms;
}
...
// ========== MV ==========
// MV 详情：名称、封面、时长、播放次数、歌手、简介等
async function mvDetail(id) {
  return call('mv_detail', { mvid: id });
}

// MV 播放地址：mv_url 同复用（已在 songUrlSmart 中用于音轨兜底）
// 这里直接导出并命名为 mvUrl（上面已定义 async function mvUrl 在 168 行附近）
